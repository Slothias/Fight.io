...
void GameEngine::GenerateWeapon()
{
    while (server->is_started())
    {
        if (players.size() > numberOfWeaponsOnTheFloor(drop_weapons) + 1)
        {
            ...
            else if (10 <= Wtype && Wtype < 25)
            { /// ~15% - 5�s fegy�
                Wtype = 4;
            }
            else if (25 <= Wtype && Wtype < 45)
            { /// ~20% - 5�s fegy�
                Wtype = 3;
            }
            else if (45 <= Wtype && Wtype < 70)
            { /// ~25% - 5�s fegy�
                Wtype = 2;
            }
            else
            ...
            std::unique_ptr<Message> message = std::make_unique<Message>(ss.str());
...
std::string GameEngine::ReSpawn(std::string name)
{
    ...
    p_mutexes[p]->lock();
    if (p->getHealthState().currentHp == 0)
    {
        float x, y, rot = 0;
        int wp = 0;
        int curhp = 100;
